...
return g.FileReader!==p&&(q||g.JSZip)!==p&&!G()&&v},text:function(a){
return a.i18n("buttons.excel","Exportar a Hoja de Cal")},action:function(a,c,d,b){
var e=0,a=function(a){
    return i.parseXML(z[a])},f=a("xl/worksheets/sheet1.xml"),
        h=f.getElementsByTagName("sheetData")[0],
        a={_rels:{".rels":a("_rels/.rels")},
    xl:{_rels:{"workbook.xml.rels":a("xl/_rels/workbook.xml.rels")},
        "workbook.xml":a("xl/workbook.xml"),
        "styles.xml":a("xl/styles.xml"),
        worksheets:{"sheet1.xml":f}},
    "[Content_Types].xml":a("[Content_Types].xml")},
        c=c.buttons.exportData(b.exportOptions),m,k,d=function(a){m=e+1;k=n(f,"row",{attr:{r:m}});
    for(var b=0,
c=a.length;b<c;b++){
    for(var d=b,g="";0<=d;)g=String.fromCharCode(d%26+65)+g,d=Math.floor(d/26)-1;
    var d=g+""+m,j=null;if(!(null===a[b]||a[b]===p||""===a[b])){a[b]=i.trim(a[b]);
        for(var l=0,o=H.length;l<o;l++)
            if(g=H[l],a[b].match&&a[b].match(g.match)){j=a[b].replace(/[^\d\.\-]/g,"");g.fmt&&(j=g.fmt(j));j=n(f,"c",{attr:{r:d,s:g.style},children:[n(f,"v",{text:j})]});break}j||("number"===typeof a[b]||a[b].match&&a[b].match(/^-?\d+(\.\d+)?$/)&&!a[b].match(/^0\d+/)?j=n(f,"c",{attr:{t:"n",r:d},children:[n(f,
